...
parentraw='/data/projects/recon/data/private/neonatal/raw/umcu_220222a/2022_02_21/';
parentproc='/data/projects/recon/data/private/neonatal/proc/umcu_220222a/';
subdirs={'GR_262175'};
...
%%
ky=r.Parameter.Labels.Index.ky_label;
kz=r.Parameter.Labels.Index.kz_label;
idx=1:length(kz);
figure
plot(kz)
...
sensemap = s_resliced;
warning('fix coil 8')
sensemap(:,:,:,8) = []; % FIXME: coil 8 disabled

% crop y and flip z
sensemap=sensemap(:,(chalf+1):(end-chalf),end:-1:1,:);

target = sum(imspace.*conj(sensemap), 4);
t=abs(target); t=t/max(t(:))*1000; % FIXME: quick scaling
nii=make_nii(t,[.38 .38 2])
save_nii(nii,fullfile(odir,'t1.nii'));
%n.dat.fname=fullfile(odir,'t1.nii');
%n.dat.dim=size(t);
%n.dat.scl_slope=1; % FIXME: adjust
%n.hdr.pixdim(2:4)=[.38 .38 2]; % FIMXE
create(n)
n.dat(:,:,:)=t;
...
imsl=imspace(256,:,:,:);
sensesl=sensemap(256,:,:,:);

%ksl=bart('fft 6',bart('fftshift 6',imsl));
ksl=bart('fft 6',imsl);
ksl=permute(ksl,[2 3 1 4]);
sensesl=permute(sensesl,[2 3 1 4]);
out=bart('pics -l2',ksl,sensesl);

dipshow(out,[0 1e-7])
%%
mask=bart('poisson -Y 404 -Z 64 -C 20 -y 1 -z 1 -v -e ');
acc=numel(mask)/sum(mask(:))

dipshow(mask)

outa=bart('pics -l1 -r 0.001 -d5 ',ksl.*repmat(squeeze(mask),[1 1 1 31]),sensesl);
dipshow(outa)
...
dipshow(sensemap(:,(chalf+1):(end-chalf),:,1))

% flip slice order
target = sum(imspace.*conj(sensemap(:,(chalf+1):(end-chalf),end:-1:1,:)), 4);
dipshow(target)
