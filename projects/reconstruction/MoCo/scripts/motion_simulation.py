...
if __name__ == "__main__":
    ...
    kspace, _, _ = apply_mask(
        kspace,
        create_mask_for_mask_type("poisson2d", [0.0], [14]),
        None,
        None,
        shift=True,
    ...
    motion_kspace = torch.view_as_real(
        torch.multiply(exp_phase_shift, torch.view_as_complex(kspace).flatten()).reshape(kspace.shape[:-1])
    )
    motion_imspace = ifft2(
        motion_kspace, centered=fft_centered, normalization=fft_normalization, spatial_dims=spatial_dims
    )
    ...
    pw_motion_imspace = ifft2(
        pw_motion_kspace, centered=fft_centered, normalization=fft_normalization, spatial_dims=spatial_dims
    )
    ...
    pw_motion_target = torch.view_as_complex(
        torch.sum(complex_mul(pw_motion_imspace, complex_conj(sensitivity_map)), coil_dim - 1)
    ).cpu()
