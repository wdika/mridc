...
def update_model_config(model_cls: MRIDCConfig, update_cfg: "DictConfig", drop_missing_subconfigs: bool = True):
    ...
    if "target" in update_cfg.model:
        ...
        if "target" not in model_cls.model:  # type: ignore
...
def _update_subconfig(
    ...
    Args:
        ...
        drop_missing_subconfigs: A bool flag, whether to allow deletion of the MRIDCConfig sub-config,
...
def _add_subconfig_keys(model_cfg: "DictConfig", update_cfg: "DictConfig", subconfig_key: str):
    ...
    update sub-config, as the actual override values might be overridden by the MRIDCConfig defaults.
