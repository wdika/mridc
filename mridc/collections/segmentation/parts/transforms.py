...
class JRSMRIDataTransforms:
    ...
    def __call__(
        ...
        target_segmentation: The target segmentation.
        ...
        if target_segmentation is not None and target_segmentation.size != 0:
            target_segmentation = torch.from_numpy(target_segmentation)
        ...
        if self.crop_size is not None and self.crop_size not in ("", "None"):
            ...
            if target_segmentation is not None:
                target_segmentation = center_crop(target_segmentation, self.crop_size)
        ...
        if self.normalize_inputs:
            ...
            if self.max_norm:
                ...
                target_segmentation = target_segmentation / torch.max(torch.abs(target_segmentation))
