...
class BaseMRIJointReconstructionSegmentationModel(base_reconstruction_models.BaseMRIReconstructionModel, ABC):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.total_segmentation_loss_weight = cfg_dict.get(
        ...
        if "cross_entropy" in segmentation_loss_fn:
            cross_entropy_loss_weight = cfg_dict.get(
                "cross_entropy_loss_weight", None)
            if not utils.is_none(cross_entropy_loss_weight):
                cross_entropy_loss_weight = torch.tensor(
        ...
        if "dice" in segmentation_loss_fn:
            self.segmentation_loss_fn["dice"] = segmentation_losses.dice.Dice(  # type: ignore
                include_background=cfg_dict.get(
            ...
            self.dice_loss_weighting_factor = cfg_dict.get(
                "dice_loss_weighting_factor", 1.0)

        self.consecutive_slices = cfg_dict.get("consecutive_slices", 1)

        cross_entropy_metric_weight = cfg_dict.get(
            "cross_entropy_metric_weight", None)
        if not utils.is_none(cross_entropy_metric_weight):
            cross_entropy_metric_weight = torch.tensor(
        ...
        self.dice_coefficient_metric = segmentation_losses.dice.Dice(  # type: ignore
            include_background=cfg_dict.get(
                "dice_metric_include_background", False),
        ...
        if self.use_reconstruction_module:
            self.total_reconstruction_loss_weight = cfg_dict.get(
        ...
        if self.segmentation_loss_fn["cross_entropy"] is not None:
            loss_dict["cross_entropy_loss"] = (
                self.segmentation_loss_fn["cross_entropy"].cpu()(
                    target.argmax(1).detach().cpu(), pred.detach().cpu())
                * self.cross_entropy_loss_weighting_factor
            )
        if self.segmentation_loss_fn["dice"] is not None:
            _, loss_dict["dice_loss"] = self.segmentation_loss_fn["dice"](
                target, pred)
            loss_dict["dice_loss"] = loss_dict["dice_loss"] * \
                self.dice_loss_weighting_factor
        loss_dict["segmentation_loss"] = loss_dict["cross_entropy_loss"] + \
            loss_dict["dice_loss"]
        return loss_dict
    ...
    def training_step(self, batch: Dict[float, torch.Tensor], batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        y, mask, init_reconstruction_pred, r = self.process_inputs(
            y, mask, init_reconstruction_pred)

        target_reconstruction = target_reconstruction / \
            torch.max(torch.abs(target_reconstruction))  # type: ignore
        ...
        if self.use_sens_net:
            ...
            if self.coil_combination_method == "SENSE":
                init_reconstruction_pred = utils.sense(
                    fft.ifft2(y, self.fft_centered,
                              self.fft_normalization, self.spatial_dims),
                    sensitivity_maps,
                    self.coil_dim,
                )
        ...
        if self.consecutive_slices > 1:
            ...
            pred_segmentation = pred_segmentation.reshape(
                batch_size * slices, *
                pred_segmentation.shape[2:]  # type: ignore
            )

        target_segmentation = target_segmentation.type(
            torch.float32)  # type: ignore
        pred_segmentation = pred_segmentation.type(torch.float32)

        segmentation_loss = self.process_segmentation_loss(
            target_segmentation, pred_segmentation)["segmentation_loss"]
    ...
    def validation_step(self, batch: Dict[float, torch.Tensor], batch_idx: int) -> Dict:
        ...
        if self.use_reconstruction_module:
            ...
            if self.consecutive_slices > 1:
                pred_reconstruction = pred_reconstruction.reshape(
                    ...
                    pred_reconstruction.shape[1], *
                    pred_reconstruction.shape[2:]
                )

            output_reconstruction = torch.abs(
                pred_reconstruction / torch.max(torch.abs(pred_reconstruction)))
            output_reconstruction = torch.abs(
                output_reconstruction).detach().cpu()

            self.log_image(f"{key}/reconstruction/prediction",
                           output_reconstruction)
            self.log_image(f"{key}/reconstruction/error",
                           target_reconstruction - output_reconstruction)
        ...
        for class_idx in range(target_segmentation.shape[1]):  # type: ignore
            ...
            target_segmentation_class = target_segmentation_class / \
                torch.max(torch.abs(target_segmentation_class))

            output_segmentation_class = pred_segmentation[:, class_idx]
            output_segmentation_class = output_segmentation_class / \
                torch.max(torch.abs(output_segmentation_class))
            ...
            self.log_image(
                f"{key}/segmentation_classes/prediction_class_{class_idx}", output_segmentation_class)
            self.log_image(
        ...
        dice_score, _ = self.dice_coefficient_metric(
            target_segmentation, pred_segmentation)
    ...
    def test_step(self, batch: Dict[float, torch.Tensor], batch_idx: int) -> Tuple[str, int, torch.Tensor]:
        ...
        predictions = (
            (pred_segmentation.detach().cpu().numpy(),
             pred_reconstruction.detach().cpu().numpy())
            if self.use_reconstruction_module
    ...
    def train_epoch_end(self, outputs):
        ...
        self.log("train_loss", torch.stack(
            [x["train_loss"] for x in outputs]).mean(), sync_dist=True)
        self.log(
            f"train_loss_{self.acc}x",
            torch.stack([x[f"train_loss_{self.acc}x"]
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log("val_loss", torch.stack(
            [x["val_loss"] for x in outputs]).mean(), sync_dist=True)
        ...
        if self.use_reconstruction_module:
            ...
            for k in self.mse_vals_reconstruction.keys():
                mse_vals_reconstruction[k].update(
                    self.mse_vals_reconstruction[k])
        ...
        for fname in dice_vals:
            ...
            metrics_segmentation["Cross_Entropy"] = metrics_segmentation["Cross_Entropy"] + torch.mean(
                torch.cat([v.view(-1).float()
                          for _, v in cross_entropy_vals[fname].items()])
            )
            metrics_segmentation["DICE"] = metrics_segmentation["DICE"] + torch.mean(
                torch.cat([v.view(-1).float()
                          for _, v in dice_vals[fname].items()])
            )
        ...
        metrics_segmentation["Cross_Entropy"] = self.CROSS_ENTROPY(
            metrics_segmentation["Cross_Entropy"])
        metrics_segmentation["DICE"] = self.DICE(metrics_segmentation["DICE"])

        if self.use_reconstruction_module:
            metrics_reconstruction["MSE"] = self.MSE(
                metrics_reconstruction["MSE"])
            metrics_reconstruction["NMSE"] = self.NMSE(
                metrics_reconstruction["NMSE"])
            metrics_reconstruction["SSIM"] = self.SSIM(
                metrics_reconstruction["SSIM"])
            metrics_reconstruction["PSNR"] = self.PSNR(
                metrics_reconstruction["PSNR"])

        tot_examples = self.TotExamples(torch.tensor(local_examples))
        for metric, value in metrics_segmentation.items():
            self.log(f"{metric}_Segmentation", value /
                     tot_examples, sync_dist=True)
        if self.use_reconstruction_module:
            for metric, value in metrics_reconstruction.items():
                self.log(f"{metric}_Reconstruction", value /
                         tot_examples, sync_dist=True)
    ...
    def test_epoch_end(self, outputs):
        ...
        for fname in predictions:
            predictions[fname] = np.stack(
                [out for _, out in sorted(predictions[fname])])
    ...
    def _setup_dataloader_from_config(cfg: DictConfig) -> DataLoader:
        ...
        dataset = segmentation_mri_data.JRSMRISliceDataset(
            ...
            segmentation_classes_to_remove=cfg.get(
                "segmentation_classes_to_remove", None),
            segmentation_classes_to_combine=cfg.get(
                "segmentation_classes_to_combine", None),
            segmentation_classes_to_separate=cfg.get(
                "segmentation_classes_to_separate", None),
            ...
            transform=transforms.JRSMRIDataTransforms(
                ...
                prewhitening_scale_factor=cfg.get(
                    "prewhitening_scale_factor", 1.0),
                prewhitening_patch_start=cfg.get(
                    "prewhitening_patch_start", 10),
                prewhitening_patch_length=cfg.get(
                ...
                coil_combination_method=cfg.get(
                ...
                kspace_zero_filling_size=cfg.get(
