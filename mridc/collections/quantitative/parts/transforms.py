...
class qMRIDataTransforms:
    ...
    def __call__(
        ...
        if self.apply_prewhitening:
            kspace = torch.stack(
                [self.prewhitening(kspace[echo]) for echo in range(
                    kspace.shape[0])],  # type: ignore
                dim=0,
            )
        ...
        if self.crop_size is not None and self.crop_size not in ("", "None"):
            ...
            if mask_brain.dim() != 0:
                mask_brain = utils.center_crop(mask_brain, self.crop_size)
        ...
        if utils.is_none(self.mask_func):
    ...
    def __init__(
        ...
        meshgrids = torch.meshgrid(
            [torch.arange(size, dtype=torch.float32)
             for size in kernel_size],  # type: ignore
            indexing="ij",
        )
