...
class EMAOptimizer(torch.optim.Optimizer):
    """
    EMAOptimizer is a wrapper for torch.optim.Optimizer that computes Exponential Moving Average of parameters
    ...
    def switch_main_parameter_weights(self, saving_ema_model: bool = False):
        ...
        for param, ema_param in zip(self.all_parameters(), self.ema_params):  # type: ignore
    ...
    def swap_ema_weights(self, enabled: bool = True):
        ...
        It swaps back to the original regular parameters on context manager exit.
