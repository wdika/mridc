...
from mridc import SSIMLoss, rss
...
def train_epoch(
    ...
    best_dev_loss: float,
    output_type: str,
) -> Tuple[float, List[Any], float, int]:
    """
    ...
    for i, data in enumerate(train_loader):
        img, target, _, _, acc, max_value, _ = data
        target = target.to(args.device)
        max_value = max_value.to(args.device)

        if isinstance(img, list):
            r = np.random.randint(len(img))
        ...
        if i % args.report_interval == 0:
            ...
            val_loss, mse_loss, nmse_loss, psnr_loss, ssim_loss, _ = evaluate(
                args, iteration, model, val_loader, loss_fn, writer, output_type
            )
