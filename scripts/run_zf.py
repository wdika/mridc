...
def run_zf(data_loader: DataLoader, fft_type: str, device: str, progress_bar_refresh: int) -> Dict:
    ...
    for i, data in enumerate(data_loader):
        (masked_kspace, sensitivity_map, _,
            ...
            estimate = torch.abs(
                torch.view_as_complex(
                    torch.sum(complex_mul(
                        ifft2c(masked_kspace, fft_type=fft_type), complex_conj(sensitivity_map)), 1)
                )
            )
...
def main(args):
    ...
    reconstructions = run_zf(data_loader, args.fft_type,
                             args.device, args.progress_bar_refresh)
